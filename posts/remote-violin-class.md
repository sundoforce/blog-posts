...
//"time"
	...
	"github.com/sundoforce/muse.ac.api/config"
	"github.com/sundoforce/muse.ac.api/controllers"
	"github.com/sundoforce/muse.ac.api/routes"
	//jwt
...
"message": "You are at the endpoint 😉",
	...
	// 합주
	routes.EnsembleRoute(api.Group("/ensembles"))
	//routes.AuthRoute(atuh.Group("/auth"))
	routes.TutorRoute(api.Group("/tutors"))
	routes.RoomRoute(api.Group("/rooms"))
}
...
app.Use(cors.New())
	...
	app.Use(cors.New(cors.Config{
		AllowOrigins: "*",
		//AllowOrigins: "http://localhost:3001, https://muse.ac, https://musia.live",
...
import (
	"github.com/gofiber/fiber/v2"
	"github.com/sundoforce/muse.ac.api/config"
	"github.com/sundoforce/muse.ac.api/models"
...
// 실시간 합주 (Sound) 방 개설
func CreateEnsemble(c *fiber.Ctx) error {
	ensembleCollection := config.MI.DB.Collection(os.Getenv("ENSEMBLE_COLLECTION"))
	data := new(models.Ensemble)
	...
	result, err := ensembleCollection.InsertOne(c.Context(), data)
	...
	ensembleCollection.FindOne(c.Context(), query).Decode(ensembles)
...
// 실시간 합주 방 목록
func GetEnsembles(c *fiber.Ctx) error {

	query := bson.D{{}}
	ensembleCollection := config.MI.DB.Collection(os.Getenv("ENSEMBLE_COLLECTION"))

	cursor, err := ensembleCollection.Find(c.Context(), query)
	...
	var ensembles []models.Ensemble = make([]models.Ensemble, 0)

	err = cursor.All(c.Context(), &ensembles)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"success": false,
			"message": "Somting went  wrong",
...
// 실시간 합주 상세 보기
func GetEnsemble(c *fiber.Ctx) error {
	ensembleCollection := config.MI.DB.Collection(os.Getenv("ENSEMBLE_COLLECTION"))
	...
	err = ensembleCollection.FindOne(c.Context(), query).Decode(ensembles)
	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"success": true,
		"data": fiber.Map{
			"ensembles": ensembles,
